...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(labels) >= max_length:
            sentence_fix = sentence[:max_length]
            labels_fix = labels[:max_length]
            mask = [True] * max_length
        else:
            sentence_fix = sentence + ([zero_vector] * (max_length - len(labels)))
            labels_fix = labels + ([zero_label] * (max_length - len(labels)))
            mask = [True] * len(labels) + [False] * (max_length - len(labels))
        ret.append((sentence_fix,labels_fix,mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(dtype=tf.int32,shape=(None,self.max_length, Config.n_features),name='input_placeholder')
        self.labels_placeholder = tf.placeholder(dtype=tf.int32,shape=(None,self.max_length),name='labels_placeholder')
        self.mask_placeholder = tf.placeholder(dtype=tf.bool,shape=(None, self.max_length),name='mask_placeholder')
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        E = tf.Variable(self.pretrained_embeddings,dtype=tf.float32,name='E')
        extracted_embeddings = tf.nn.embedding_lookup(E,self.input_placeholder,name='extracted_embeddings')
        embeddings = tf.reshape(extracted_embeddings,(-1,self.max_length, self.config.n_features * self.config.embed_size),name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op_rnn(self, x, dropout_rate):
        ...
        b2 = tf.get_variable('b2', (1, Config.n_classes),
                            dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables() #setting reuse flag to true
                o_t, h_t = cell(x[:,time_step,:],h)
                o_drop_t = tf.nn.dropout(o_t, dropout_rate,name='o_drop_t')
                y_t = tf.add(tf.matmul(o_drop_t,U),b2,name='y_t')
                preds.append(y_t)
                ### END YOUR CODE
    ...
    def add_prediction_op_gru(self, x, dropout_rate):
        ...
        with tf.variable_scope("GRU"):
            ...
            o_t, _ = tf.nn.dynamic_rnn(GRU, x, dtype=tf.float32)
            y_t = tf.layers.dense(o_t,Config.n_classes,
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        #tiled_mask = tf.tile(tf.expand_dims(self.mask_placeholder,axis=2),[1,1,self.config.n_classes])
        cross_entropy_pred = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,
                                                                            logits=preds,
                                                                            name='cross_entropy_pred')
        loss = tf.reduce_mean(tf.boolean_mask(cross_entropy_pred,self.mask_placeholder),name='loss')
        ### END YOUR CODE
        return loss
    ...
    def add_summary_op(self, pred, loss):
        ...
        self.probs = tf.nn.softmax(pred,name='probs')
        entropy =  tf.reduce_mean((- self.probs * tf.log(tf.clip_by_value(self.probs,1e-4,1))),name='entropy')
        records.append(tf.summary.histogram('pred_histogram', pred))
        records.append(tf.summary.scalar('loss',loss))
        records.append(tf.summary.scalar('entropy', entropy))
        ### END YOUR CODE
