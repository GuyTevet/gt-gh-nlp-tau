...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x',(self.input_size,self.state_size),dtype=tf.float32,
                            initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable('W_h',(self.state_size,self.state_size),dtype=tf.float32,
                            initializer=tf.contrib.layers.xavier_initializer())
            b   = tf.get_variable('b',(self.state_size),dtype=tf.float32,
                            initializer=tf.constant_initializer())
            new_state = tf.nn.sigmoid(tf.matmul(state,W_h)+tf.matmul(inputs,W_x)+b,name='new_state')
            ### END YOUR CODE ###
